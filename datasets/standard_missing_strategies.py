...
def missing_by_top_corr_column(df, uic, pic, pa_names, label_names):
    col_name, col_index = get_most_corr_column(df, label_names, pa_names)
    return get_missing_matrix_by_column(df, uic, pic, pa_names, col_index)


def missing_by_column(df, uic, pic, pa_names, col_name):
    col_index = df.columns.get_loc(col_name)
    return get_missing_matrix_by_column(df, uic, pic, pa_names, col_index)


def missing_single_col_by_sample(df, uic, pic, pa_names, label_names):
    return get_missing_matrix_by_samples(
        df, uic, pic, label_names, pa_names, rand_single_col_selector)


def missing_all_cols_by_sample(df, uic, pic, pa_names, label_names):
    return get_missing_matrix_by_samples(
        df, uic, pic, label_names, pa_names, rand_many_col_selector)
...
def get_missing_matrix_by_column(df, uic, pic, pa_names, col_index):
    ...
    for r, p in zip([0, 1], [uic, pic]):
        ...
        choices = np.random.choice(indices, size=n_missing,
                                   replace=False,)
...
def get_missing_matrix_by_samples(df, uic, pic, label_names, pa_names,
                                  column_selector, **kwargs):
    n_samples = df.shape[0]
    n_features = df.shape[1] - 1
    missing_matrix = np.zeros((n_samples, n_features))

    for i, (index, row) in enumerate(df.iterrows()):
        if have_missing(row, uic, pic, pa_names):
...
def have_missing(row, uic, pic, pa_names):
    toss = np.random.rand()
    if row[pa_names[0]] == 1:
        return toss >= (1 - pic)
    else:
...
def rand_many_col_selector(row, uic, pic, pa_names, label_names, cutoff=0.5):
    ...
    toss = np.random.rand(len(indices))
